...
def draw_bboxes(ax, bboxes, color='w', alpha=0.8, thickness=2):
...
def draw_labels(ax,
    ...
    for i, (pos, label) in enumerate(zip(positions, labels)):
        ...
        if scores is not None:
            label_text += ":"+f'{scores[i]:1.02f}'
            # label_text += '.%2.f'% (100 * scores[i]) + "%"
...
def draw_masks(ax, img, masks, color=None, with_edge=True, alpha=0.8):
    ...
    for i, mask in enumerate(masks):
        ...
        if with_edge:
            ...
            for c in contours:
                polygons += [Polygon(c)]
                border_color += [color_mask/255]

    p = PatchCollection(polygons, facecolor='none', edgecolors=border_color, linewidths=1.2,alpha=0.8)
    ax.add_collection(p)
...
def imshow_det_bboxes(img,
    ...
    if bboxes is not None:
        ...
        draw_bboxes(ax, bboxes, bbox_color, alpha=0.8, thickness=thickness)
    ...
    if segms is not None:
        ...
        draw_masks(ax, img, segms, mask_color, with_edge=True,alpha=alpha)
