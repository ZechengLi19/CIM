...
from tools.RW_tools.graph_tools import *
import seaborn as sns
from tools.RW_tools.mmcv_box.visualization.image import imshow_det_bboxes,imshow_gt_det_bboxes
from matplotlib.patches import Polygon
from pycocotools import mask as maskUtils
from pycocotools._mask import frPoly
...
def normalize_cam(class_response_maps_normalize):
    class_response_maps_normalize = (class_response_maps_normalize - class_response_maps_normalize.min((-2, -1))[..., None, None]) / \
        ...
        if self.inferencing:
            ...
            for cls_idx in lables:
                ...
                for idx in range(peak_list_cls.size(0)):
                    peak_val = class_response_maps[peak_list_cls[idx, 0],  peak_list_cls[idx, 1], peak_list_cls[idx, 2], peak_list_cls[idx, 3]]
                    #print(peak_val)
                    if peak_val > peak_threshold:
                        exist = 1
                        grad_output.zero_()
                        # starting from the peak
                        grad_output[peak_list_cls[idx, 0], peak_list_cls[idx, 1], peak_list_cls[idx, 2], peak_list_cls[idx, 3]] = 1
                ...
                if exist == 0:
                    peak_val_list = [class_response_maps[0, cls_idx, peak_list_cls[idx, 2], peak_list_cls[idx, 3]] for idx in range(peak_list_cls.size(0))]
...
voc_id_name_map = {1:'aeroplane',2:'bicycle',3:'bird',4:'boat',5:'bottle',6:'bus',7:'car',8:'cat',9:'chair',10:'cow',
...
class PeakResponseMapping_aff(nn.Sequential):
    ...
    def get_peak_list(self,class_response_maps,lables,peak_threshold,peak_filter=None):
        if self.enable_peak_stimulation:
            ...
            # aggregate responses from informative receptive fields estimated via class peak responses
            if peak_filter == None:
                peak_list = peak_stimulation_aff(class_response_maps, win_size=self.win_size,peak_filter=self.peak_filter)
            else:
        ...
        if peak_list is None:
            if peak_filter == None:
                ...
                peak_list = peak_stimulation_aff(class_response_maps, return_aggregation=False, win_size=self.win_size,peak_filter=peak_filter)
            # peak_list = peak_stimulation_aff(class_response_maps, return_aggregation=False, win_size=self.win_size,peak_filter=self.peak_filter)
...
class PeakResponseMapping_aff_visual(nn.Sequential):
    ...
    def forward(self, input, lables, img,class_threshold=0, peak_threshold=10, retrieval_cfg=None,graph_op=None,resolution=7):
        ...
        with torch.no_grad():
            ...
            for idx in range(1):
                if idx == 0:
                    img_col = cv2.resize(img, (int(cam_W*self.sub_pixel_locating_factor),int(cam_H*self.sub_pixel_locating_factor)))

                    plt.close()
                    plt.imshow(np.uint8(img))
                    plt.axis("off")

                    plt.savefig(os.path.join(dir, str(img_id) + "org" + ".jpg"), dpi=300,bbox_inches='tight',pad_inches=0)

                elif idx == 1:
                    sp_label = cv2.resize(np.uint8(sp_label), (int(cam_W*self.sub_pixel_locating_factor),int(cam_H*self.sub_pixel_locating_factor)),cv2.INTER_NEAREST)
                    ...
                    img_col = np.concatenate((img_col,cv2.resize(gt_img, (int(cam_W*self.sub_pixel_locating_factor),int(cam_H*self.sub_pixel_locating_factor)))),axis=1)
                    ...
                    plt.axis("off")
                    plt.savefig(os.path.join(dir, str(img_id) + "proposal_sum" + ".jpg"), dpi=300,bbox_inches='tight',pad_inches=0)

                    res = cv2.resize(res, (int(cam_W*self.sub_pixel_locating_factor),int(cam_H*self.sub_pixel_locating_factor)))
                    img_col = np.concatenate((img_col,res),axis=1)

                else:
                    img_col = np.concatenate((img_col,255 * np.ones((int(cam_W*self.sub_pixel_locating_factor),int(cam_H*self.sub_pixel_locating_factor),3))),axis=1)
                ...
                for i,cls_idx in enumerate(lables):
                    ...
                    for peak in VPL:
                        if peak[1] == cls_idx:
                            result = cv2.circle(result,center=(int(peak[3]),int(peak[2])),radius=2,color=(0,0,255),thickness=-1)
                    ...
                    if idx == 0:
                        ...
                        plt.savefig(os.path.join(dir, str(img_id), str(img_id) + "heat" + ".jpg"), dpi=300, bbox_inches='tight',pad_inches=0)
            ...
            for i, cls_idx in enumerate(lables):
                cv2.putText(result,VOC_CLASSES[cls_idx] , (5, (i)*120 + 180), cv2.FONT_HERSHEY_COMPLEX, 0.8, (255, 0, 0), 1)
