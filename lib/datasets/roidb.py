...
def combined_roidb_for_training(dataset_names, proposal_files, mat_files):
    ...
    """
    def get_roidb(dataset_name, proposal_file, mat_file):
        ds = JsonDataset(dataset_name)
        roidb = ds.get_roidb(
            gt=True,
            proposal_file=proposal_file,
            mat_file = mat_file,
            crowd_filter_thresh=cfg.TRAIN.CROWD_FILTER_THRESH # 0
        )
    ...
    if len(mat_files) == 0:
        mat_files = (None, ) * len(dataset_names) 
    assert len(dataset_names) == len(proposal_files)
    roidbs = [get_roidb(*args) for args in zip(dataset_names, proposal_files, mat_files)]
    roidb = roidbs[0]
    for r in roidbs[1:]:
        roidb.extend(r)
